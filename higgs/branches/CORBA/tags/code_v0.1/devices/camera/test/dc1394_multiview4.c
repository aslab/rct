...
{
    if (text == NULL) {                         // if there's no text, do nothin
	return;
    }
    glPushAttrib(GL_LIST_BIT);                  // alert that we're about to off
    glListBase(base - 32);                      // sets the base character to 32

    glCallLists(strlen(text), GL_UNSIGNED_BYTE, text); // draws the display list
    glPopAttrib();                              // undoes the glPushAttrib(GL_LI
}
...
void InitGL(int Width, int Height)              // We call this right after our OpenGL window is created.
{
  ...
  glTexImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, GL_RGBA, videoImageWidth,
  	videoImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, videoTexture);
}
...
void DrawGLScene()
{
  ...
  //yuv422_2vuy422(frames[0]->image, videoTexture, 640, 480);
  //memcpy(videoTexture, frames[0]->image, 640*480*2);
  //(uint8_t*)frame_buffer1,
  dc1394_deinterlace_stereo((uint8_t*)frames[0]->image,
		frame_buffer1,
		device_width, device_height*2);
  dc1394_bayer_decoding_8bit((uint8_t*)frame_buffer1,
		frame_buffer2,
		device_width, device_height,
...
/*
  dc1394_convert_to_YUV422((uint8_t*)frame_buffer1, (uint8_t*)frame_buffer1,
	device_width, device_height,
...
*/
  ...
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
  //glOrtho( (GLfloat)0, (GLfloat)videoImageWidth, (GLfloat)0, (GLfloat)videoImageHeight, -1.0, 1.0);
  gluPerspective(45.0f,(GLfloat)videoImageWidth/(GLfloat)videoImageHeight,1.0f,1200.0f);
  float viewerPosition[3]={-320.0, -240.0, -600.0};
  ...
  glEnable(GL_TEXTURE_RECTANGLE_ARB);
  glBindTexture(GL_TEXTURE_RECTANGLE_ARB, texture[0]);
  memcpy(videoTexture, frame_buffer2, videoImageWidth*videoImageHeight*3);
  //dc1394_convert_to_RGB8((uint8_t*)frame_buffer1, videoTexture,
  //		    videoImageWidth, videoImageHeight,
  //		    DC1394_BYTE_ORDER_UYVY, DC1394_COLOR_CODING_YUV422,16);
//  glTexSubImage2D (GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, videoImageWidth, videoImageHeight, 
//	GL_YCBCR_422_APPLE, GL_UNSIGNED_SHORT_8_8_REV_APPLE, videoTexture);
  glTexSubImage2D (GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, videoImageWidth, videoImageHeight, 
...
void set_frame_length(unsigned long size, int numCameras)
{
    ...
    frame_free=0;
    printf("tamaño %d \n", size);
    frame_buffer1 = malloc( size * numCameras * 4);
    frame_buffer2 = malloc( size * numCameras * 4);
    videoTexture  = malloc( size * numCameras * 4);
}
