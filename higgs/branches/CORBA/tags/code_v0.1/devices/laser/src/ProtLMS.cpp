// ProtLMS.cpp: implementation of the ProtLMS class.
//
//////////////////////////////////////////////////////////////////////
...
ProtLMS::ProtLMS()
{
    ...
    for(int i=0;i<401;i++)
      {
	sen_alfa[i]=(float)sin((40.0f+i*0.25f)*PI/180.0f);
	cos_alfa[i]=(float)cos((40.0f+i*0.25f)*PI/180.0f);
      }
...
int ProtLMS::Init(const char device[])
{
	...
	if (i == size) // In case a Reset() returns the laser to 9600bps.
	    ...
	    bps = baud[i];
	port.SetBaud(bps);
	printf("Trying to connect at %d baud.\n",bps);
...
// Call it before changing the baudrate.
