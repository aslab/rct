...
int PortCOM::Init (const char device[]) 
{
#ifdef WIN32
...
int PortCOM::Receive ( int num,unsigned char * cad) 
{
#ifdef WIN32
    ...
    FD_ZERO(&rfs);FD_SET(serialPort,&rfs);
    /*int r = */select(serialPort+1,&rfs,NULL,NULL,&to);
...
int PortCOM::Send (int n,unsigned char * cad) 
{
#ifdef WIN32
