// -*- C++ -*-
//
// $Id$

// ****  Code generated by the The ACE ORB (TAO) IDL Compiler ****
// TAO and the TAO IDL Compiler have been developed by:
//       Center for Distributed Object Computing
//       Washington University
//       St. Louis, MO
//       USA
//       http://www.cs.wustl.edu/~schmidt/doc-center.html
// and
//       Distributed Object Computing Laboratory
//       University of California at Irvine
//       Irvine, CA
//       USA
//       http://doc.ece.uci.edu/
// and
//       Institute for Software Integrated Systems
//       Vanderbilt University
//       Nashville, TN
//       USA
//       http://www.isis.vanderbilt.edu/
//
// Information about TAO is available at:
//     http://www.cs.wustl.edu/~schmidt/TAO.html


// TAO_IDL - Generated from
// ../../../ACE_wrappers/TAO/TAO_IDL/be/be_visitor_interface/interface_ci.cpp:70

#if !defined (_COSNAMING_NAMINGCONTEXT___CI_)
#define _COSNAMING_NAMINGCONTEXT___CI_

ACE_INLINE
CosNaming::NamingContext::NamingContext (
    TAO_Stub *objref,
    ::CORBA::Boolean _tao_collocated,
    TAO_Abstract_ServantBase *servant,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (objref, _tao_collocated, servant, oc),
    the_TAO_NamingContext_Proxy_Broker_ (0)
{
  this->CosNaming_NamingContext_setup_collocation ();
}

ACE_INLINE
CosNaming::NamingContext::NamingContext (
    IOP::IOR *ior,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (ior, oc),
    the_TAO_NamingContext_Proxy_Broker_ (0)
{
}

#endif /* end #if !defined */

// TAO_IDL - Generated from
// ../../../ACE_wrappers/TAO/TAO_IDL/be/be_visitor_interface/interface_ci.cpp:70

#if !defined (_COSNAMING_BINDINGITERATOR___CI_)
#define _COSNAMING_BINDINGITERATOR___CI_

ACE_INLINE
CosNaming::BindingIterator::BindingIterator (
    TAO_Stub *objref,
    ::CORBA::Boolean _tao_collocated,
    TAO_Abstract_ServantBase *servant,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (objref, _tao_collocated, servant, oc),
    the_TAO_BindingIterator_Proxy_Broker_ (0)
{
  this->CosNaming_BindingIterator_setup_collocation ();
}

ACE_INLINE
CosNaming::BindingIterator::BindingIterator (
    IOP::IOR *ior,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (ior, oc),
    the_TAO_BindingIterator_Proxy_Broker_ (0)
{
}

#endif /* end #if !defined */

// TAO_IDL - Generated from
// ../../../ACE_wrappers/TAO/TAO_IDL/be/be_visitor_interface/interface_ci.cpp:70

#if !defined (_COSNAMING_NAMINGCONTEXTEXT___CI_)
#define _COSNAMING_NAMINGCONTEXTEXT___CI_

ACE_INLINE
CosNaming::NamingContextExt::NamingContextExt (
    TAO_Stub *objref,
    ::CORBA::Boolean _tao_collocated,
    TAO_Abstract_ServantBase *servant,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (objref, _tao_collocated, servant, oc),
    the_TAO_NamingContextExt_Proxy_Broker_ (0)
{
  this->CosNaming_NamingContextExt_setup_collocation ();
}

ACE_INLINE
CosNaming::NamingContextExt::NamingContextExt (
    IOP::IOR *ior,
    TAO_ORB_Core *oc
  )
  : ::CORBA::Object (ior, oc),
    the_TAO_NamingContextExt_Proxy_Broker_ (0)
{
}

#endif /* end #if !defined */

